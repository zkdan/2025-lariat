...
export default function SinglePhoto() {
  ...
  // Parse the filename to determine if it's a cover or monthly image
      ...
      >
        {imageData ? (
          <div className="relative inline-block">
            <div className={`absolute w-[600px] h-full bg-gray-200 animate-pulse transition-opacity duration-500 ease-in-out ${imageLoaded ? 'opacity-0' : 'opacity-100'}`}></div>
            <Image
              src={imageData.fullUrl}
              width={600}
              height={400}
              alt={`Calendar image for ${imageData.name} ${imageData.year}`}
              className={`drop-shadow-[25px_25px_35px_rgba(0,0,0,0.5)] transition-opacity duration-500 ease-in-out ${imageLoaded ? 'opacity-100' : 'opacity-0'
                }`}
